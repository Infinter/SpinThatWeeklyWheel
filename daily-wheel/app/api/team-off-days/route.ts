...
// Proxy d'ÉCRITURE pour `team_off_days` (AD-7, AD-14). SEUL endroit qui importe le client secret pour cette
// table. Contrat : POST { op: 'insert'|'delete', id?, data? } + header x-team-passphrase. PAS d'`update` :
// on ajoute et on supprime des jours off unitairement (epics.md#Story-3.3). La clé secrète contourne RLS
// (AD-9) ; la garde passphrase (AD-8) est l'unique verrou réel. Mirroir de la route unavailabilities pour
// la validation jour/plage. AUCUNE contrainte d'unicité sur cette table → pas de cas 23505 dédié, pas de
// message « doublon » ; `label` est OPTIONNEL (colonne nullable).
export const runtime = 'nodejs'

// Allowlist de colonnes écrivables pour `team_off_days` (AD-14). id/updated_at = serveur.
const ALLOWED = ['kind', 'date1', 'date2', 'label'] as const
...
// Mappe une erreur Supabase/Postgres vers la taxonomie HTTP (AD-17). Pas de contrainte d'unicité sur cette
// table → un 23505 improbable retombe dans le mapping générique (409 sans message « doublon »).
...
// Validation serveur DÉFENSIVE (AD-17:400) — dernière ligne : un caller direct ne doit pas insérer une plage
// inversée. La validation primaire est cliente/pure (AC5). Normalise `picked` in place : day → date2 forcé
// null ; label optionnel → trim, vide ou non-string → null. Retourne un message d'erreur, ou null si valide.
function validateInsert(picked: Record<string, unknown>): string | null {
  ...
  if (typeof picked.date1 !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(picked.date1)) {
    return 'date1 requise (chaîne YMD)'
  }
  if (kind === 'range') {
  ...
  // label OPTIONNEL (colonne nullable) : trim si chaîne, vide → null ; absent/non-string → null.
  picked.label = typeof picked.label === 'string' && picked.label.trim() !== '' ? picked.label.trim() : null
  return null
}
...
export async function POST(request: Request): Promise<Response> {
  ...
  const table = supabaseAdmin.from('team_off_days')
  ...
  if (Object.keys(picked).length === 0) {
    return json(400, {
      error: 'data vide après allowlist (colonnes autorisées : kind, date1, date2, label)',
    })
  }
