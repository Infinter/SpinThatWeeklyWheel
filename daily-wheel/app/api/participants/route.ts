...
// Proxy d'ÉCRITURE pour `participants` (AD-7, AD-14). SEUL endroit qui importe le client secret.
// Contrat unique : POST { op: 'insert'|'update'|'delete', id?, data? } + header x-team-passphrase.
// La clé secrète contourne RLS (AD-9) ; la garde passphrase (AD-8) est l'unique verrou réel.
export const runtime = 'nodejs'

// Allowlist de colonnes écrivables pour `participants` (AD-14). id/created_at/updated_at = serveur.
const ALLOWED = ['name', 'active'] as const
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (op !== 'insert' && op !== 'update' && op !== 'delete') {
    return json(400, { error: "op invalide (attendu: 'insert' | 'update' | 'delete')" })
  }

  const table = supabaseAdmin.from('participants')
  ...
  // --- INSERT / UPDATE : allowlist appliquée AVANT écriture (AD-14) ---
  const picked = pickAllowed(body.data)
  if (Object.keys(picked).length === 0) {
    return json(400, { error: 'data vide après allowlist (colonnes autorisées : name, active)' })
  }

  if (op === 'insert') {
    // id/created_at/updated_at : générés/positionnés serveur (défauts SQL — AD-15).
    const { data: row, error } = await table.insert(picked).select().single()
    if (error) return mapDbError(error)
    return json(200, row)
  }

  // op === 'update' : patch partiel + updated_at repositionné serveur (le default SQL
  // ne s'applique qu'à l'insert → on le fixe explicitement ici, AD-15).
  if (typeof body.id !== 'string' || !body.id) return json(400, { error: 'id requis pour update' })
